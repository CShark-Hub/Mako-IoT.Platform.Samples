...
namespace MakoIoT.Device.PlatformClient.App.HardwareServices
{
    public class ConfigButton
    {
        ...
        public ConfigButton(IMediator mediator, ILogger logger, GpioController gpio)
        {
            _mediator = mediator;
            _logger = logger;

            var configModeButton = new GpioButton(gpio: gpio, buttonPin: 18);
            configModeButton.Press += ConfigModeButton_Press;

            var resetToDefaultsButton = new GpioButton(gpio: gpio, buttonPin: 19);
